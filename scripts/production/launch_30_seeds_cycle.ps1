# launch_30_seeds_cycle.ps1
# Lancement parallele de 30 seeds CYCLE (300 trials chacun)
# Usage: .\scripts\launch_30_seeds_cycle.ps1
...
$OUT_BASE = Join-Path $ROOT "outputs\wfa_phase_cycle"
...
Write-Host "  LANCEMENT 30 SEEDS CYCLE - WFA Phase-Adapte" -ForegroundColor Cyan
Write-Host "========================================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path $LABELS_CSV)) {
    ...
    Write-Host "Executez d'abord: py scripts/generate_cycle_labels.py" -ForegroundColor Yellow
    exit 1
}
